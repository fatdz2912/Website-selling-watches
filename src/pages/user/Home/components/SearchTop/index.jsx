import { Carousel, Skeleton, Space, Col } from 'antd'
...
function SearchTop() {
  ...
  const renderSearchTop = useMemo(() => {
    if (loading) {
        ...
        <S.SearchTopList gutter={[16, 16]}>
          {[...Array(4)].map((_, index) => (
            <Col key={index} md={6} sm={12} xs={12}>
              <S.SearchTopsItem style={{ padding: 8 }}>
      ...
      return groupSearchTop.map((group, index) => {
        return (
          <S.SearchTopList key={index} gutter={[20, 20]} justify={'space-between'}>
  ...
  return (
    <S.SearchTopWrapper>
      ...
      <Carousel style={{ width: '100% ' }} autoplay dots dotPosition={'top'} autoplaySpeed={2500}>
        {renderSearchTop}
      </Carousel>
    </S.SearchTopWrapper>
  )
